app.controller('InvestmentProofController', [
    '$scope', '$q', '$timeout', '$modal', '$mdDialog', '$routeParams', '$location', 'utilityService', 'ServerUtilityService', 'TaxService', 'TimeOffService','EmployeeTaskService',
    function ($scope, $q, $timeout, $modal, $mdDialog, $routeParams, $location, utilityService, serverUtilityService, taxService, timeOffService, EmployeeTaskService) {
        var self = this;
        ...
        $scope.empDetails = {
            name: utilityService.getValue($routeParams, 'name'),
            code: utilityService.getValue($routeParams, 'code'),
            pic: utilityService.getValue($routeParams, 'pic'),
            year: utilityService.getValue($routeParams, 'year')
        };
        $scope.calenderDate = {
            datepicker_min_date: new Date('04/01/' + utilityService.getValue($routeParams, 'year')),
            datepicker_from_max_date: new Date('03/31/' + (parseInt(utilityService.getValue($routeParams, 'year'), 10) + 1)),
            datepicker_to_max_date: new Date('03/31/' + (parseInt(utilityService.getValue($routeParams, 'year'), 10) + 1)),
        }
        $scope.pageType = utilityService.getValue($routeParams, 'fnf_type', false) ? 'fnf' : 'other'
        ...
        var getInvestmentDetails = function(isSaveAndNext) {
            var claimUrl = taxService.getUrl('adminClaimtax') + "/"
                + utilityService.getValue($routeParams, 'empId') + "/" 
                + parseInt($scope.empDetails.year) + "/" + (parseInt($scope.empDetails.year) + 1)
        ...
        $scope.saveRentDetails = function(rentReceipt, panCard, isSaveAndNext) {
            ...
            $scope.errorMessages = [];
            var url = taxService.getUrl('adminRentDetail') + "/" 
                    + utilityService.getValue($routeParams, 'empId') + "/"
                    + $scope.rentDetail.tax_id,
        ...
        $scope.deleteHRADetails = function (detail, item) {
            var url = taxService.getUrl('adminDeleteHRADetails') + "/" 
                + utilityService.getValue($routeParams, 'empId') + "/"
                + item._id.$id;
            serverUtilityService.deleteWebService(url)
                .then(function (data) {
        ...
        $scope.saveUpdateInvestmentHead = function (item, event) {
            ...
            var url = taxService.getUrl('adminInvestmentHead') + "/"
                    + utilityService.getValue($routeParams, 'empId') + "/" 
        ...
        $scope.viewDownloadClaimProof = function(item, action, slug) {
        ...
        $scope.goBack = function () {
            var tabname =  utilityService.getValue($routeParams, 'employee_type') === 'active' 
                    ? 'Investment Proofs' : 'full-and-final';
        ...
        $scope.getTaxRegime = function () {
            var Url = taxService.getUrl('taxRegimePayroll') + "/" + parseInt($scope.empDetails.year) + "/" + (parseInt($scope.empDetails.year) + 1) + "/" + utilityService.getValue($routeParams, 'empId');
            serverUtilityService.getWebService(Url)
                .then(function (data) {
                    $scope.taxRegime = data.data;
        ...
        $scope.approveHRAAmount = function (detail, hraPendingAmount) {
            var url = EmployeeTaskService.getUrl('approveAmount') + "/" + detail.details[0].allowance_id,
                payload = {
                    ...
                    start_year: parseInt($scope.empDetails.year),
                    end_year : parseInt($scope.empDetails.year) + 1                 
                };
